...
class SpellingDatabase(object):
    def __init__(self):
        ...
        if len(database_check) < 5:
            ...
            self.sql("""CREATE TABLE users (user_id INTEGER PRIMARY KEY,
                                               username varchar(30),
                                               passwd varchar(64))""")

            self.sql("""CREATE TABLE user_list_map (user_id INTEGER,
                                                    list_id INTEGER,
                                                    best_score INTEGER,
                                                    best_time INTEGER)""")
    ...
    def getWords(self, listName):
        return self.sql("""SELECT words.* FROM words, lists, word_list_map where lists.list_id = word_list_map.list_id 
